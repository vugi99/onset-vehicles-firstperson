local last_veh_supported = 36 -- put 0 here to enable compatibility mode for all vehicles (you'll have weird camera locations sometimes but it's ok)
    ...
    if (IsPlayerInVehicle() and IsFirstPersonCamera() and myseat) then
        ...
        if last_veh_supported >= id then
            ...
            elseif ((GetVehicleModel(veh) == 30 or GetVehicleModel(veh) == 31) and (myseat == 1 or myseat == 2)) then
                decal = -45
                height = 56
            elseif ((GetVehicleModel(veh) == 30 or GetVehicleModel(veh) == 31) and (myseat == 3 or myseat == 4)) then
                decal = -35
                height = 90
            end
            SetCameraLocation(x+ux*height+fx*decal+rx*decal_r, y+uy*height+fy*decal+ry*decal_r, z+uz*height+fz*decal+rz*decal_r ,true)
        else
            local bx, by, bz = GetPlayerBoneLocation(GetPlayerId(), "head") -- compatibility mode
            SetCameraLocation(bx, by, bz, true)
        end
        if viewlock then
